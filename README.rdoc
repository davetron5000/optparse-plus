...
A canonical `OptionParser` driven app has a few problems with it strucgturally that methadone can solve
...
Methadone gives you a simple,lightweight DSL to help.  It's a work in progress, but here's what you have right now:
...
+go!+ runs the block you gave to +main+, passing it the unparsed +ARGV+ as parameters to the block.  
